...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        else if (prefs.getString("default_screen", "notes") == "calendar" && savedInstanceState == null) {
    ...
    override fun onResume() {
        super.onResume()
        sortMode = prefs.getString("sort", sortMode) ?: sortMode
        viewMode = prefs.getString("view", viewMode) ?: viewMode
        if (::list.isInitialized) applyViewMode()
        if (::sortBar.isInitialized) updateSortBar()
    ...
    private fun buildUi(): LinearLayout {
        ...
        val top = LinearLayout(this).apply {
            ...
            setPadding(dp(12), dp(7), dp(5), dp(7))
        }
        ...
        top.addView(action("⋮") { showMainMenu() })
        root.addView(top)
        ...
        sortBar = TextView(this).apply {
            ...
            background = rounded(WoWPalette.SURFACE, dp(8).toFloat(), WoWPalette.LINE, dp(1))
        ...
        search = EditText(this).apply {
            ...
            background = rounded(WoWPalette.CARD, dp(10).toFloat(), WoWPalette.LINE, dp(1))
        ...
        val fab = TextView(this).apply {
            ...
            textSize = 36f
            gravity = Gravity.CENTER
            setTextColor(WoWPalette.BG)
            background = rounded(WoWPalette.ACCENT, dp(22).toFloat(), WoWPalette.LINE, dp(1))
            elevation = dp(10).toFloat()
            setOnClickListener { chooseNewNote() }
            contentDescription = "Add note"
        }
        content.addView(fab, FrameLayout.LayoutParams(dp(66), dp(66), Gravity.END or Gravity.BOTTOM).apply {
            setMargins(0, 0, dp(16), dp(14))
        })
    ...
    private fun bottomNav(): LinearLayout {
        ...
        bar.addView(item("▤", !pinnedOnly && !archivedMode) {
            pinnedOnly = false
            archivedMode = false
            hideSearch()
            updateHeader()
    ...
    private fun showMainMenu() {
        AlertDialog.Builder(this).setItems(arrayOf("View", "Sort", "Archive", "Calendar", "Settings", "Custom font")) { _, which ->
            when (which) {
                0 -> showViewDialog()
                1 -> showSortDialog()
                2 -> { archivedMode = true; pinnedOnly = false; hideSearch(); updateHeader(); loadNotes("") }
                3 -> startActivity(Intent(this, CalendarActivity::class.java))
                4 -> startActivity(Intent(this, SettingsActivity::class.java))
                5 -> fontPicker.launch(arrayOf("font/ttf", "font/otf", "application/x-font-ttf", "application/octet-stream"))
            }
        }.show()
    }
    ...
    private fun updateHeader() {
        title.text = when { archivedMode -> "Archive"; pinnedOnly -> "Pinned"; else -> "WoW Note" }
    }
    ...
    private fun applyViewMode() {
        ...
        list.layoutManager = when (viewMode) {
            "grid", "large_grid" -> GridLayoutManager(this, 2)
    ...
    private fun noteMenu(note: Note) {
        AlertDialog.Builder(this).setTitle(note.title.ifBlank { "Untitled" })
            .setItems(arrayOf("Open", "Color", if (note.pinned) "Unpin" else "Pin", if (note.archived) "Unarchive" else "Archive", "Delete")) { _, which ->
                when (which) {
                    0 -> openNote(note)
                    ...
                    4 -> AlertDialog.Builder(this).setMessage("Delete this note?")
